...
**API 키 없이 결과만 보고 싶다면**: 앱 첫 화면의 **📦 샘플 데이터 불러오기** 버튼을 누르면 됩니다. [`sample_data/`](sample_data)에 미리 담아둔, 실제로 AI가 끝까지 처리한 결과(가상 부서 20개·대분류 32건)가 즉시 로드되어 대시보드·대분류 카드·부서별 탭을 전부 정상적으로 둘러볼 수 있습니다. 비용도, 대기 시간도, 키 등록도 필요 없습니다 
