...
- **한계점**: 현재 직접 API 경로(`ai_pipeline.py`)는 전 단계 Sonnet 모델로 고정되어 있어, 원래 설계된 모델 티어 정책(§4, 분류·추천 Opus / 검증 Sonnet)과 결과물 품질 차이가 발생하고 Claude Code 서브에이전트 경로보다 처리 시간도 더 걸린다. 고객사가 실제 업무 판단을 이 결과에 의존하려면, 판단 품질이 핵심인 분류·추천 단계에서 이 정도 품질 격차는 그대로 두고 넘어가기 어렵다.
  - **개선방안**: 고객사별로 전용 API 사용량(자체 API 키 제공 또는 사용량 기반 과금)을 배정해 분류·추천 단계를 Opus 티어로 제공한다. 초기 도입 비용을 낮추고 싶은 고객사를 위해서는, 판단 품질이 가장 중요한 분류(③ account-classifier) 단계만 Opus로 전환하는 하이브리드 티어링을 기본 플랜으로 제공해 비용 증가를 최소화하면서 품질 격차를 줄이는 방안도 병행할 수 있다.
