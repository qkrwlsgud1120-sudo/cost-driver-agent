...
→ 이 중 2번 문제를 해결하기 위해 공통/특정 계정을 구분하는 구조를 도입했습니다. 이 프로젝트는 세 문제를 아래와 같이 구조적으로 풀어냅니다.

먼저 전체 부서 파일을 사전에 스캔해 계정 인벤토리를 만들고, 계정코드가 아니라 **대분류**(계정명에서 도출한 카테고리) 단위로 공통/특정 여부를 먼저 확정합니다(Phase 0 → Phase 0.5). 공통 대분류는 한 번만 판단하면 그 아래 세부계정 전체에 "원칙 준용"으로 자동 상속되므로, 부서마다 같은 계정을 반복 판단하는 문제(2번)가 구조적으로 사라집니다. 그다음 `account-classifier`(4-type 분류) → `driver-recommender`(원가동인 1~3순위 추천) → `result-validator`(자기검증) 세 개의 전문화된 Claude 서브에이전트가 보험업 도메인 지식과 ABC 원가계산 프레임워크를 참조해 판단과 근거를 생성해 도메인 지식 병목(1번)을 완화합니다. 판단 불일치·휴먼 에러(3번)는 모든 판단(AI든 회계사 직접 확정이든)에 근거 문장과 출처(`분류근거`·`근거출처`)를 필수로 남기고, 계정 단위로 처리 상태(`처리완료`)를 추적해 중단 후 재개해도 이미 처리된 계정은 건너뛰고 미완료 계정만 이어서 처리하는 구조로 줄입니다.
