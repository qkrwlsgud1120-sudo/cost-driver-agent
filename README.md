...
같은 판단 지침을 두 경로가 공유하므로 원칙적으로 결과 품질은 동일해야 하지만, 서브에이전트 경로는 사람이 중간 산출물을 검토할 수 있어 실사용에는 이 경로를 우선합니다. 직접 API 경로는 GitHub 링크만으로 제3자가 재현해볼 수 있게 하기 위한 별도 구현이며, 모델 티어를 낮춘 것도 같은 이유입니다 — 자소서에 제출된 링크는 면접관이 언제·몇 번 누를지 전혀 통제할 수 없어 비용 예측 가능성이 우선이고, Opus 품질의 실제 판단 결과는 이미 `docs/screenshots`·`docs/project_summary.md`에 남아있어 이 경로가 최고 품질을 낼 필요는 없습니다.
