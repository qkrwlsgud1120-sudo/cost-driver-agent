...
- **한계점**: 현재 공개된 직접 API 경로(`ai_pipeline.py`)는 전 단계 Sonnet 모델만 쓰도록 고정되어 있다. 이는 불특정 다수가 접근하는 데모 환경에서 비용을 예측 가능하게 만들기 위한 임시 조치였고, 원래 설계된 모델 티어 정책(§4, 분류·추천 Opus / 검증 Sonnet)과는 결과물 품질 차이가 있다. 실제로 이 도구를 특정 보험사에 상용 공급한다면, 계약된 고객사 없이 불특정 방문자를 가정한 지금의 비용 안전장치(세션당·일일 호출 한도, Sonnet 고정)를 그대로 쓸 수 없다 — 실사용 고객은 데모 방문자가 아니라 실제 업무에 이 판단 품질을 신뢰해야 하는 회계사이기 때문이다.
  - **개선방안**: 상용 계약 고객사 단위로 별도 API 사용량(자체 API 키 또는 사용량 기반 과금)을 배정해, 고객사별로 분류·추천 단계는 Opus 티어로 정상 제공한다. 이는 다른 SaaS형 AI 도구에서도 흔히 쓰이는 BYOK(Bring Your Own Key) 또는 사용량 종량제 방식과 같은 구조다. 초기 도입 비용이 부담되는 고객사를 위해서는, 판단 품질이 가장 중요한 분류(③ account-classifier) 단계만 Opus로 전환하는 하이브리드 티어링을 기본 플랜으로 제공해 비용 증가를 최소화하면서 품질 격차를 줄이는 방안도 병행할 수 있다.
