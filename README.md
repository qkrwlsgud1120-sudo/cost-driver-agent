...
관리회계 비용실사에서 각 비용 계정에 원가동인을 설정하는 작업에는 다음과 같은 Pain Point가 있습니다.

1. **도메인 지식 부재로 인한 상류 단계 지연**: 보험업에 대한 도메인 지식 부재로 인해 특유의 계정 성격(재보험비 항목, 보험 계약 관련 비용 구조 등)을 파악하는 데 많은 시간이 소요되고, 가장 상류 단계인 계정 분류부터 지연되어 이후 전체 파이프라인이 늦어집니다.
2. **부서별 순차 검토로 인한 중복 판단 비효율**: 부서별로 파일을 순차적으로 검토하는 과정에서, 타 부서와 동일한 성격의 계정을 중복 판단하는 비효율이 발생합니다.
3. **동일 계정에 대한 판단 불일치(휴먼 에러)**: 같은 계정에 대해 서로 다른 원가동인이 선정되거나, 워딩(표현)이 불일치하는 휴먼 에러가 발생합니다.

이 프로젝트는 세 문제를 아래와 같이 구조적으로 풀어냅니다.

가장 핵심은 **부서별로 순차 실사하지 않는다는 것**입니다. 먼저 전체 부서 파일을 사전에 스캔해 계정 인벤토리를 만들고, 계정코드가 아니라 **대분류**(계정명에서 도출한 카테고리) 단위로 전체 비용을 한 번에 스크리닝해 공통/특정 여부를 먼저 확정합니다(Phase 0 → Phase 0.5). 확정된 대분류 아래 세부계정 전체는 그 판단을 "원칙 준용"으로 자동 상속받습니다. 이 구조 하나로 2번과 3번이 동시에 풀립니다 — 대분류를 부서마다 반복 판단하지 않으므로 중복 판단 비효율(2번)이 사라지고, 같은 대분류는 전체 부서를 통틀어 단 한 번만 판단되어 그대로 통일 적용되므로 부서마다 다른 원가동인이 선정되거나 워딩이 갈릴 여지 자체가 구조적으로 없어집니다(3번). 그다음 `account-classifier`(4-type 분류) → `driver-recommender`(원가동인 1~3순위 추천) → `result-validator`(자기검증) 세 개의 전문화된 Claude 서브에이전트가 보험업 도메인 지식과 ABC 원가계산 프레임워크를 참조해 판단과 근거를 생성해 도메인 지식 병목(1번)을 완화합니다. 모든 판단(AI든, 세부계정을 개별적으로 예외 지정한 회계사 직접 확정이든)에는 근거 문장과 출처(`분류근거`·`근거출처`)가 함께 기록되어 추적 가능합니다.

AI는 추천과 근거 제시까지만 담당하고, 최종 확정은 항상 회계사가 Streamlit 화면에서 승인·수정·직접입력으로 결정합니다(human-in-the-loop).
...
**직접 업로드해서 실시간으로 돌려보고 싶다면** (선택 사항): 레포 루트의 [`부서별_비용_최종 샘플.xlsx`](부서별_비용_최종%20샘플.xlsx)를 내려받아 그대로 업로드하면 바로 시도해볼 수 있습니다.
...
업로드부터 Phase 1(4-type 분류·원가동인 추천)까지 실제 화면이 어떻게 이어지는지 단계별 전/후 스크린샷으로 캡처했습니다.

### 1. 부서 파일 업로드 → 분석 시작 (Phase 0)

사이드바에 부서별 비용 파일을 업로드하면 20개 부서가 자동 인식됩니다. 이 상태에서 **「분석 시작」** 버튼을 누르면 계정 인벤토리 스캔(Phase 0)이 즉시 실행됩니다.

**실행 전**
![업로드 완료, 분석 시작 전](docs/screenshots/process_01_upload.png)

**실행 후**
![분석 시작 후 Phase 0 완료](docs/screenshots/process_02_phase0_done.png)

「분석 시작」을 클릭하면 전체 20개 부서·173건의 세부계정이 스캔되어 32개 대분류로 묶이고, 그중 규칙 기반으로 즉시 판별 가능한 9건(공통)·11건(특정)은 바로 분리되지만, 부서별 금액 편차 등으로 판단이 애매한 12건은 "Phase 0.5 재확인 대기" 상태로 남습니다.

### 2. Phase 0.5 — AI 공통/특정 판정

규칙 기반으로 분리하지 못한 12건은 「전체」 탭 상단의 **🤖 AI 공통/특정 판정 시작 (12건)** 버튼을 눌러 해소합니다. 클릭하면 각 대분류가 여러 부서에 걸쳐 실질적으로 같은 원가 발생 구조를 공유하는지를 AI가 판단해 "공통유지" 또는 "특정전환"으로 확정합니다.

**실행 전 (재확인 대기 12건)**
![Phase 0.5 재확인 대기, 버튼 클릭 직전](docs/screenshots/process_03_phase05_before.png)

**실행 후 (Phase 0.5 완료)**
![Phase 0.5 완료 후 대시보드](docs/screenshots/process_04_phase05_done.png)

버튼 클릭 한 번으로 12건이 모두 해소되어 공통 대분류 9건 → 18건, 특정 대분류 11건 → 14건으로 재배치되고, "Phase 0.5" 카드가 완료 상태로 바뀝니다. 이 시점부터 공통 대분류 18개 × 평균 12.4개 부서 = 223건의 개별 판단을 18건으로 축소(92% 절감)한 효과가 대시보드에 집계됩니다.

### 3. Phase 1 — AI 분류·원가동인 추천

Phase 0.5가 끝나면 「전체」 탭에 **🤖 AI 분류·원가동인 추천 시작 (32건)** 버튼이 나타납니다. 이 버튼을 누르면 32개 대분류 전체에 대해 4-type 분류와 원가동인 1~3순위 추천이 한 번에 실행됩니다.

**실행 전 (32건 대기)**
![Phase 1 실행 전, 버튼 클릭 직전](docs/screenshots/process_05_phase1_before.png)

**실행 후 (분류·추천 완료)**
![Phase 1 실행 후 4-type 분포](docs/screenshots/process_06_phase1_done.png)

실행 후에는 32개 대분류 중 25건이 공통비형·배부형·직접귀속형 중 하나로 명확히 분류되고, 세부계정 성격이 이질적이거나 자기신뢰도가 낮은 7건은 4-type을 강제 배정하지 않고 "추가판단 필요" 상태로 남아 회계사 확인을 기다립니다.

### 4. 회계사 확정 — 대분류 원가동인 선택과 세부계정 예외 지정

원가동인 확정은 **대분류 단위**로 이루어집니다. driver-recommender가 대분류마다 1순위 원가동인과 함께 2~3순위 대안을 근거와 함께 제시하면, 회계사는 화면에서 그대로 살펴보고 확정합니다.

![대분류 카드에서 1순위 승인 또는 다른 추천안(2순위) 확인](docs/screenshots/process_07_category_driver_confirm.png)

기본은 **1순위 승인**이지만, 1순위보다 2순위(또는 3순위)가 이 데이터셋의 실제 상황에 더 목적적합하다고 판단되면 「다른 추천안 보기」로 펼쳐 그 순위를 확정할 수 있고, AI 추천 어느 것도 적절하지 않으면 원가동인을 직접 작성해 확정할 수도 있습니다.

대분류 하위의 세부계정들은 대부분 그 대분류와 동일한 비용 성격을 공유하므로, 기본적으로는 대분류에서 확정한 원가동인을 그대로 상속받습니다("원칙 준용" — 세부계정마다 다시 판단할 필요가 없습니다). 다만 특정 세부계정만 고유한 성격 때문에 대분류 원칙과 다른 원가동인이 더 적합하다면, 그 계정만 개별적으로 **예외로 지정**해 원가동인을 직접 작성할 수 있습니다.

![세부계정별 원칙 준용 상태와 예외로 지정 버튼](docs/screenshots/process_08_subaccount_exception.png)
...
같은 판단 지침을 두 경로가 공유하므로 원칙적으로 결과 품질은 동일해야 하지만, 서브에이전트 경로는 사람이 중간 산출물을 검토할 수 있어 실사용에는 이 경로를 우선합니다. 직접 API 경로는 GitHub 링크만으로 제3자가 재현해볼 수 있게 하기 위한 별도 구현입니다. 두 경로의 모델 티어 차이가 만드는 실제 품질·속도 격차는 아래 [한계점 및 개선방안](#한계점-및-개선방안)에 정리했습니다.
...
- **한계점**: 공개 배포 버전(Streamlit Cloud, 직접 API 경로)은 Claude Code 서브에이전트가 아니라 Anthropic API 직접 호출 방식으로 동작하며, 이 경로는 전 단계 Sonnet 모델만 사용한다. 원래 설계된 모델 티어 정책(§4, 분류·추천 Opus / 검증 Sonnet)과 결과물 품질 차이가 발생하고, Claude Code 서브에이전트 경로보다 처리 시간도 더 걸린다.
  - **개선방안**: 배포 화면에 "본인 API 키로 실행" 옵션을 추가해, 방문자가 원하면 자신의 키로 Opus 티어를 선택해 돌려볼 수 있게 한다. 또는 판단 품질이 가장 중요한 분류(③ account-classifier) 단계만 Opus로 전환하는 하이브리드 티어링을 적용해, 비용 증가를 최소화하면서 품질 격차를 줄인다.

- **한계점**: Phase 1(4-type 분류·원가동인 추천)은 대분류 하나당 분류→추천→검증 최대 3회의 LLM 호출이 필요해, 처리할 대분류·세부계정 수가 늘어날수록 크레딧(API 비용) 사용량이 그만큼 늘어난다. 지금은 가상 샘플(20개 부서·32개 대분류) 규모라 부담이 크지 않지만, 실제 고객사 데이터처럼 부서·계정 수가 훨씬 많아지면 한 번의 전체 실행에 드는 비용이 커져 실사용 시 비용 부담이 늘어난다.
  - **개선방안**: 대분류를 금액 규모 기준으로 우선순위화해 비중이 큰 대분류만 Opus로 정밀 처리하고, 소액·저빈도 대분류는 더 저렴한 모델이나 규칙 기반 폴백으로 처리하는 비용 계층화(cost tiering)를 도입한다. 이미 판단이 끝난 계정명 패턴은 재사용해 동일 판단에 대한 중복 호출도 줄인다.

- **한계점**: 현재 구조는 회계사 한 명이 Streamlit 세션 하나에서 확정 결과를 저장하는 것을 전제로 한다(confirmed_results.json → accounts_master.json 순 반영). 동시 편집 잠금이나 변경 병합 로직이 없어, 여러 회계사가 같은 시점에 각자 다른 부서·대분류를 나눠 확정하면 나중에 저장한 사람의 결과가 앞선 결과를 덮어쓸 수 있다. 이는 실제 팀 단위 상용화의 한계로 이어진다.
  - **개선방안**: `accounts_master.json`을 단일 파일이 아니라 DB(SQLite/Postgres 등)로 옮기고 대분류 단위로 낙관적 잠금 또는 트랜잭션을 적용해 동시 저장 충돌을 감지·병합한다. 우선은 대분류를 회계사별로 배정해 담당자만 해당 대분류를 확정할 수 있도록 제한하는 간단한 역할 분리부터 도입할 수 있다.
