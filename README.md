...
- **한계점**: AI 실행 사용량 제한이 "세션당 실행 횟수"만 카운트하고, 실행 1회가 처리하는 대분류 수에는 상한이 없다. 대분류 32개가 남아있으면 버튼 1번으로 32개 전부(분류·추천·검증 합쳐 최대 96회 API 호출)가 처리되어, 세션당 실행 횟수 제한(3회)만으로는 실질적인 비용 상한이 되지 않는다.
  - **개선방안**: 실행 1회당 처리 가능한 대분류 수에 별도 상한(예: 20건)을 추가하고, 남은 대분류는 다음 실행에서 이어서 처리하도록 한다.

- **한계점**: 회계사 확정 반영 후 반환되는 재작업 목록(`재실행_필요_recommend`/`재실행_필요_validate_only`)이 Streamlit 화면에 노출되지 않는다. 현재는 이 목록을 코드 실행 결과나 로그로 직접 확인해야 알 수 있다.
  - **개선방안**: "전체 저장" 직후 화면에 재작업 대상 대분류 목록을 배너로 표시하고, 「전체」 탭의 AI 버튼이 그 목록만 우선 처리하도록 연결한다.
