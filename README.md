...
- **한계점**: 현재는 판단 근거 자료(ABC 원가계산 원칙·4-type 분류체계·보험회계 일반 지식, 3개 문서 총 168줄·약 1만 8천자)를 매 호출 시스템 프롬프트에 통째로 포함하는 방식이다. 이는 코퍼스가 작고 고정돼 있어 가능한 구조로, 고객사 한 곳의 정형화된 참조 자료 범위 안에서는 문제가 없다. 하지만 여러 고객사를 동시에 지원하며 고객사마다 다른 내부 회계 정책·과거 실사 이력을 참조해야 하거나, 참조 자료 자체가 수백 페이지 규모의 규정집으로 커지면 전체 포함 방식은 컨텍스트에 다 들어가지 않거나, 들어가더라도 매 호출마다 불필요한 토큰을 반복 전송하는 비효율로 이어진다.
  - **개선방안**: 참조 자료를 벡터DB에 임베딩해두고, 판단마다 실제로 관련된 조항만 검색해 프롬프트에 포함하는 RAG(Retrieval-Augmented Generation) 구조로 전환한다. 고객사별로 별도 문서 저장소를 두면 멀티 테넌시도 자연스럽게 지원되고, 참조 자료가 갱신돼도 재배포 없이 최신 내용이 바로 검색에 반영된다.
